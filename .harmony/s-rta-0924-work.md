...
- 09:32 lock cleared (Harmony_Main frozen battery pid 58189 held ~35 min, blocked this repo's builder dispatch — same class as the pending report filed earlier). Onset-count builder dispatched.
- ONSET COUNT fix db72d56 (FeatureSnapshot::onsetCount @316, sizeof 320; RecorderHost count-delta, cap 8/tick with carry): reviewer APPROVE-WITH-NITS (nits: first tick after arm is baseline -> a real onset in that ~8 ms window is swallowed, undocumented; no dedicated cap/wrap tests). Merged; build rc0; ctest 406/406 (Harmony).
