...
- graphify node counts are extractor-specific: at the s-rta-0923 close the EOS wrapper's native code-only extract wrote 6206 nodes while `graphify update .` on the same HEAD wrote 7473 (7254 at boot, +new code). Compare counts only within one extractor; the graph was left on the `update` build.

## 2026-09-24 — RecorderHost destructor needs unique_ptr<incomplete-type> fix | A class holding `std::unique_ptr<X>` where X is only forward-declared in the header MUST declare `~Class();` in the header and define it (even `= default`) in the .cpp where X's full definition is visible — otherwise every TU that includes the header and constructs the class fails to compile (`sizeof` applied to incomplete type via `std::default_delete`), even though the class itself never uses X directly outside the .cpp. | discovered: src/recording/RecorderHost.h/.cpp (HostSink forward-declared, sink_ is unique_ptr<HostSink>)

## 2026-09-24 — R13 lane A (AnalysisResampler) fail-first values matched the plan's predictions almost exactly | With `AnalysisResampler::setSourceRate` forced to permanent bypass (one-line temp hack, reverted before commit), the new T2 rate-invariance tests went RED with: T2.1 centroid@16kHz = 3016.3Hz (plan predicted ~3000), T2.3 BPM@44.1kHz = 131.84 (plan's own measured control was 131.9), T2.2 LUFS delta = 3.11 LU. This is strong independent confirmation the resampler math (ratio = sourceRate/48000, JUCE LagrangeInterpolator via `process(ratio, in, out, numOut)`) is correctly wired — a real aubio BPM lock also succeeded first-try on a synthetic LCG-noise-burst click train (120 BPM, -6dBFS, 20ms decaying bursts) at all three test rates, no tuning needed. | discovered: src/analysis/AnalysisResampler.cpp, tests/test_integration_pipeline.cpp (T2.1-T2.4)

## s-rta-0924 learnings
- **Re-read the file after an agent says "fixed".** A probe-fix agent reported the opacity rows fixed; `grep` showed lines 293/295 untouched. Receiver-verify is a file read, not a report read.
- **A SKIP is not a pass — compare row counts between runs.** probe-step3 run from a worktree with no `.venv` skipped T2 + the Output-window row and still printed "58 PASS / 0 FAIL" (prior runs: 62-63 rows). Now those rows FAIL loudly when the dependency is missing.
- **An inferred root cause is a hypothesis.** "1-sample impulse" was the stated cause of zero onset markers; a 5 ms burst also gave zero. The real cause (aubio's confirming hop landing in true digital silence) came only from an offline harness replaying the app's exact OnsetDetector config.
- **aubio onsets need a noise floor between transients.** The detector confirms a peak one hop late and gates it if that hop is -inf dB. Test fixtures: -50 dBFS floor. Real audio is fine; gated/sparse electronic material with true digital silence could lose onsets.
- **One-hop pulses through an always-latest bus get lost.** Any consumer that reads FeatureBus::read() slower than or unsynchronised with the 94 Hz analysis will miss `onsetDetected`; use the monotonic `onsetCount` and act on deltas.
- **Nested heredoc in `$(...)` breaks on an odd apostrophe count (bash 3.2).** A probe edit with "take's" in a python comment inside `$(... <<'EOF' ...)` failed `bash -n` far from the cause.
- **Live-app agents: no debugger, no GUI input.** lldb attach raises a Touch ID dialog; an Escape sent to dismiss it hit another terminal. Rule in gotchas.md.
- **Onset pulse direction flips with reader rate; the oracle is two equal counters (s-rta-0924b onset lane).** A render-cadence reader of the raw bool over a 20 s click train saw 25 of 57 onsets at 60 fps and 83 at 120 fps (`tests/test_integration_pipeline.cpp`, "Onset pulse" case). Fixed via `src/features/OnsetPulse.h`, one instance per bus-reader thread; live oracle `/api/status.renderOnsetPulses` delta == `/api/features.onsetCount` delta.
- **The default build never compiles `src/test/TestServer.cpp`.** It is gated on `-DAUDIODNA_BUILD_TEST_SERVER=ON`; a lane touching TestServer needs a second configure (e.g. `build-lane-ts`, `--target AudioDNA`, ~1 min on a warm JUCE tree) to compile-check it. Also: `ctest -j6` flaked `ThumbnailCache re-put at the same mtime` once (passes isolated and serial) -- run the authoritative count serially.

## s-rta-0924b — Bluetooth startup crash located (2026-09-24)
- **ASan names the Bluetooth startup crash: JUCE 8.0.4 CoreAudio audioCallback (juce_CoreAudio_mac.cpp:795) overruns its own temp input buffer** on a 16 kHz HFP headset. Method that worked: rebuild build-asan on the merged tree, launch the raw binary with `ASAN_OPTIONS=...:log_path=/tmp/asanN` and WAIT for the process to exit on its own — pkill at a fixed 20 s cut the report before the stack was symbolised.
...
## 2026-09-25 s-rta-0925 replay-restore (D4 preamble) | Files: src/recording/Program.{h,cpp}, Player.{h,cpp}, RecorderHost.{h,cpp}, PerfState.h, PerfStateCapture.cpp, src/MainComponent.{h,cpp}, .harmony/probe-step3.sh, tests/{test_program_preamble.cpp,test_take.cpp,test_recorder_host.cpp}
**Note:** A worktree branched "from current main" can still be MISSING same-day content that only exists as an UNCOMMITTED edit in the primary checkout -- this lane's dispatch packet said "Boris ruled 2026-09-25 (binding-decisions.md)" and cited exact line ranges, but `.harmony/binding-decisions.md` in this worktree (HEAD e501b00) has NO 2026-09-25 section at all (ends at the 2026-09-24 Bluetooth ruling); the plan/critic reports that quote it (`.harmony/.reports/s-rta-0925/plan-roadmap.md`, `critic-roadmap.md`) don't exist in this worktree's git history either -- they were only readable via an absolute path into the PRIMARY checkout (`/Users/boriskarpman/projects/RealTimeAudio/...`, not the worktree path), meaning Harmony's planning session wrote them uncommitted in the main tree while this worktree was already checked out. Also: local `main` (6d55fc9) is 2 commits ahead of `origin/main`/this worktree's HEAD (e501b00) -- an unrelated lane (`lane/visualfix`) landed on main after this worktree branched. Consequence: do NOT assume a worktree sees "current main" as of dispatch time -- diff `git rev-parse main` vs `git rev-parse HEAD` early, and treat any packet citation into `.harmony/` docs/reports as unverified until grepped IN THE WORKTREE, not just trusted from the packet text. This is why the Builder skipped the plan's `.harmony/APP-INVENTORY.md` Record-row and `binding-decisions.md` "TO VERIFY"->"BUILT" edits (§3.9 items 2-3) -- the target text to anchor on does not exist in this branch; only the CLAUDE.md one-sentence addition (§3.9 item 1, anchored on stable, non-stale content) was made.
**Valid while:** the multi-worktree dispatch model keeps spawning workers from a `main` snapshot that can lag a live main-checkout session's uncommitted docs/reports.

## 2026-09-25 s-rta-0925 mastersignal Step 0 (effect/source-param twins render-live) | Files: src/model/Clip.h, src/connect/ConnectionEngine.cpp, src/render/CompositorEngine.cpp, src/render/Renderer.{h,cpp}, src/ui/{EffectStackView,ClipInspector,UniversalParamControl,MacroPanel}.{h,cpp}, tests/test_effect_stack_binding.cpp (new)
**Note:** A JUCE `Component` is invisible by default (`componentFlags(0)` in its constructor) even when added via `addChildComponent()` -- ONLY `addAndMakeVisible()` or an explicit `setVisible(true)` makes it visible. A headless widget test asserting a display push gated on `isVisible()` (the L9 cost-trap pattern used throughout this UI layer) must force the control visible itself; it does NOT come visible "for free" just because the parent view/component tree was never given a size (that only skips `resized()`'s explicit `setVisible(false)` branch -- it does not flip the default false to true). Also folded a critic BLOCKING finding (`MacroPanel::refresh()`'s second, un-hoisted `MacroBank::updateValues()` call site, reached from a live ~10Hz Inspector-refresh timer) into this lane as a zero-behavior-change-today preventive fix, since it followed the exact same "display-only tick" pattern this lane was already establishing for `EffectStackView`/`ClipInspector`.
**Valid while:** these files exist in their current form; the JUCE default-invisible-Component fact is a JUCE library invariant, not project-specific, and holds regardless of future refactors.
