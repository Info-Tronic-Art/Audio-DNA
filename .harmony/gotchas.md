...
### 2026-09-05 — a sweep that samples one monotonic segment cannot tell two different signals apart
Source: s166 — Harmony's own live-app gate mis-identified `Mod 2`, and the mislabel reached a committed test before an independent reviewer caught it (HIGH)
Trigger: you are about to identify a signal, curve, or shape from a sequence of sampled values — or to hand such a sample to another agent as an "anchor"
Rule: sweeping `beatPhase` 0 / 0.25 / 0.5 / 0.75 returned 0.0000 / 0.1250 / 0.2500 / 0.3750 for `Mod 2`, which reads unmistakably as a SawUp oscillator at `beatDuration_ = 2`. It is actually `EnvelopeSignal("Mod 2", 4.0f)` (`SignalRegistry.cpp:70`) — a triangle whose RISING LIMB equals `beatPhase / 2` over exactly the range that was sampled. Every sampled point agreed; the identification was still wrong, and the arithmetic built on it then produced a phantom "anomaly" (a reading of 1.0000 that is simply the triangle's peak at `cyclePhase = 0.5`). **Sample points that DISCRIMINATE between the candidate shapes — the peak, the turnaround, past the halfway point — not points that are convenient or evenly spaced.** And when handing an observation to another agent, give the OBSERVATION and its PROVENANCE ("the signal named Mod 2 read back these values via `/api/signals`") separately from any identification, and make the agent derive the identity from the registration site. An authoritative-sounding label from the lead WILL be encoded as fact — here it became a test named "live-app regression anchors" that constructed the wrong class and guarded nothing while passing.
Scope: universal
Promoted: no

### 2026-09-05 — never assert on the TEXT of a JSON response, and never grep an interpolated variable that might be empty
Source: s166 — Harmony's own behavioral gate for the composition-tier oracle reported 7 failures on its first run; 5 were the gate's bugs and 2 of its passes were false
Trigger: you are writing a shell gate that checks an HTTP/JSON response
Rule: three distinct ways the same gate lied. (1) `grep '"ok":false'` never matched because this app pretty-prints JSON — the real bytes are `"ok": false` with a space, so every correctly-rejected request was scored as "silently accepted". (2) `grep '0.42'` failed against the echoed value `0.419999986886978` — a float round-trip is not a string match. (3) An extraction that returned an EMPTY string was interpolated straight into `grep "$EFF"`, and `grep ""` matches every line — so a readback check PASSED while proving nothing, and a 25-cycle stress test reported success while every single add had been rejected for a missing name. **Parse the JSON (`python3 -c 'import json,sys; d=json.load(sys.stdin)'`) and assert on typed values with a tolerance for floats; and guard any extracted variable with an explicit "abort if empty" before it reaches a pattern.** The failure mode is not that the gate breaks — it is that the gate reports confidently in both directions while measuring nothing.
Scope: universal
Promoted: no

### 2026-09-05 — this app runs TWO HTTP servers on two different addresses, and they serve different routes
Source: s166 — three probe runs lost to 404s and connection failures before the layout was established
Trigger: you are writing any script that drives Audio-DNA over HTTP
Rule: `TestServer` listens on **`http://[::1]:8080`** (IPv6 loopback, requires `--test-mode`) and owns `/api/health`, `/api/signals`, `/api/inject_features`, and the composition-tier routes (`add_global_effect`, `remove_global_effect`, `set_global_effect_bypass`, `global_effects`, `set_composition_params`, `composition_params`, `set_clip_opacity`). `ApiServer` listens on **`http://127.0.0.1:7070`** (IPv4 — it does NOT answer on `[::1]` at all) and owns `/api/effects`, `/api/sources`, `/api/load_source`, `/api/render_frame`. **A gate that drives this app needs BOTH base URLs.** Symptom of getting it wrong: a 404 (right host, wrong server) or a bare curl failure with an empty body (wrong address family) — neither of which reads as "wrong port", and the empty body is indistinguishable from a dead server, which this repo already documents as a separate trap.
Scope: repo
Promoted: no
