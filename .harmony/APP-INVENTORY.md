...
**11 OSC patterns** (subsystem INERT) · 19 binding actions · 6 feedback presets ·
**109 unit tests** (all PASS).
...
| Surface | Reach / trigger | User-visible functions | Live? |
|---|---|---|---|
| TopBar (`TopBar.h:12`) | Always visible (top, 34px) | Audio-source combo (Mic/File); input-gain slider; **Play/Pause/Stop (UNWIRED)**; Tap-tempo; Resync; manual-BPM toggle + BPM edit; 5 multiplier buttons (/4 /2 x1 x2 x4); Quantize combo; Fade slider; Master slider; Output-display combo; beat wheel + bar/phrase + FPS/DSP readouts | yes |
...
| Tab | State | Functions |
|---|---|---|
| General | wired | Show-Tooltips (Confirm-on-quit toggle removed Wave 0) |
...
### OSC input (`src/osc/OscHandler.cpp`) — 11 patterns, subsystem **INERT**

`startListening()` is never called → receiver never connects; and only 5/11 callbacks are
wired (`MainComponent.cpp:1127-1141`). Patterns (all no-op at runtime):
`/audiodna/clip/{layer}/{column}`, `/layer/{n}/opacity|bypass|solo|mute`, `/deck/{n}`,
`/master`, `/bpm`, `/snapshot`, `/macro/{n}`, `/effect/{name}/{param}`.

### Persistence (JSON via juce::var) — **LOSSY** (lane-4 F5)

| Entity | Path | Dropped on save |
|---|---|---|
| Clip | `Clip::toVar` (Clip.cpp:3-80) | clipOpacity, per-clip transform (pos/scale/rotation/anchor), blendOverride, alphaType, RGBA channels, MilkDrop preset playlist, EffectSlot.dryWet, clipW/H, beatDivision/videoBeats (when no sequenceFiles) |
| Layer | `Layer::toVar` (Layer.cpp:3-65) | per-layer transform, FeedbackConfig (all), transitionMode/blendMode, layerW/H, autoSize, autopilotLoops, autopilotEndOfVideo |
| Composition | `Composition::toVar` (Composition.h:162-198) | masterSpeed, compOpacity, crossfader (all), comp transform, composition autopilot, PerTypeAutopilotConfig, all genre-aware fields |
...
- **Tooltips**: `juce::TooltipWindow` (600ms); coverage sparse (TopBar, ClipInspector,
  LayerInspector only); toggle in Preferences → General.
...
| OSC subsystem | INERT — `startListening()` never called; 5/11 callbacks wired | OscHandler.cpp:15; MainComponent.cpp:1127-1141 |
...
| TopBar transport buttons | UNWIRED — Play/Pause/Stop declared/added, no onClick | TopBar.h:63 |
...
| 48kHz hardcode | RISK — `kSampleRate=48000` + K-weighting coeffs + all freq math, no runtime SR validation | AnalysisThread.h:46; LoudnessAnalyzer.cpp:13-29 |
...
| Waveform snapshot torn read | RISK — count published with release, buffer via plain memcpy | AnalysisThread.cpp:340-343 |
